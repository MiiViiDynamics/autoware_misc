...
#include "autoware_misc/actuation_message_converter/actuation_message_converter_node.hpp"
...
: Node("actuation_message_converter", options),
  m_string_sub(create_subscription<std_msgs::msg::String>(
    "/miivii_sensors_status", rclcpp::QoS{1},
    std::bind(&MessageConverterNode::on_state_callback_1, this, std::placeholders::_1))),
    ...
    "/control/trajectory_follower/control_cmd", rclcpp::QoS{1}.transient_local()))
{
  ...
  dds_entity_t command_topic = dds_create_topic(
    participant, &autoware_auto_vehicle_msgs_msg_VehicleControlCommand_desc, "ctrl_cmd", NULL,
    NULL);
  if (command_topic < 0) {
  ...
  // dds_qset_deadline(qos, DDS_MSECS(10)); // 设置截止时间为1000毫秒
  ...
  dds_entity_t state_topic = dds_create_topic(
    participant, &autoware_auto_vehicle_msgs_msg_VehicleKinematicState_desc, "current_pose", NULL,
    NULL);
  if (state_topic < 0) {
  ...
  dds_entity_t trajectory_topic = dds_create_topic(
    participant, &autoware_auto_planning_msgs_msg_Trajectory_desc, "trajectory", NULL, NULL);
  ...
  dds_entity_t hello_topic =
    dds_create_topic(participant, &HelloWorldData_Msg_desc, "HelloWorldData_Msg", NULL, NULL);
  if (hello_topic < 0) {
    fprintf(stderr, "dds_create_topic: %s\n", dds_strretcode(hello_topic));
    std::exit(EXIT_FAILURE);
  }

  m_hello_writer = dds_create_writer(participant, hello_topic, NULL, NULL);
  if (m_hello_writer < 0) {
    fprintf(stderr, "dds_create_writer: %s\n", dds_strretcode(-m_hello_writer));
    std::exit(EXIT_FAILURE);
  }
#if 1
...
static void copy(
  const builtin_interfaces::msg::Duration & src, builtin_interfaces_msg_Duration & dst)
{
  dst.sec = src.sec;
  dst.nanosec = src.nanosec;
}
...
static void copy(const Odometry & src, autoware_auto_vehicle_msgs_msg_VehicleKinematicState & dst)
{
...
void MessageConverterNode::on_state_callback_1(const std_msgs::msg::String msg)
{
  //   autoware_auto_vehicle_msgs_msg_VehicleKinematicState idlc_msg {};
  //   copy(*rosidl_msg, idlc_msg);
  //   dds_return_t rc = dds_write(m_state_writer, &idlc_msg);
  ...
  // RCLCPP_ERROR(get_logger(), "dds_write: %s", dds_strretcode(-rc));
  //   }
  HelloWorldData_Msg hello_msg;
  hello_msg.userID = 1;
  hello_msg.message = (char *)msg.data.c_str();
  RCLCPP_INFO(get_logger(), "dds_write: %s", msg.data.c_str());

  dds_return_t rc = dds_write(m_hello_writer, &hello_msg);
  if (rc < 0) {
    RCLCPP_ERROR(get_logger(), "dds_write: %s", dds_strretcode(-rc));
  }
}
